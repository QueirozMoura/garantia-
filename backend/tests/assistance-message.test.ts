...
let nextResponse: ProviderResponse = { kind: 'value', value: validMessage };
...
const postMessage = (purchaseId: string, token: string, body: unknown) =>
  api()
...
describe('POST /purchases/:purchaseId/assistance/message', () => {
  beforeAll(async () => {
    await cleanDatabase();

    // Replace only `getAIProvider` in the real ai.service module, so the real
    // `generateAssistanceMessage` (Zod validation + error mapping) still runs.
    vi.resetModules();
    vi.doMock('../src/services/ai/ai.service.js', async (importOriginal) => {
      ...
      return {
        ...
        getAIProvider: () => ({
          ...
          generateAssistanceMessage: async (input: Record<string, unknown>) => {
            capturedInput = input;
  ...
  afterEach(() => {
    capturedInput = null;
    nextResponse = { kind: 'value', value: validMessage };
  });
  ...
  it('sem autenticação → 401 AUTH_REQUIRED', async () => {
    const response = await api()
      .post(`/purchases/${crypto.randomUUID()}/assistance/message`)
  ...
  it('purchaseId inválido → 400 INVALID_PURCHASE_ID (sem chamar a IA)', async () => {
    ...
    const response = await postMessage('not-a-uuid', token, { problem: validProblem });
  ...
  it('problem menor que 5 caracteres → 400', async () => {
    ...
    const response = await postMessage(purchase.id, token, { problem: 'abcd' });
  ...
  it('problem maior que 2000 caracteres → 400', async () => {
    ...
    const response = await postMessage(purchase.id, token, { problem: 'a'.repeat(2001) });
  ...
  it('compra inexistente → 404 PURCHASE_NOT_FOUND', async () => {
    ...
    const response = await postMessage(crypto.randomUUID(), token, { problem: validProblem });
  ...
  it('compra de outro usuário → 403 PURCHASE_ACCESS_DENIED (sem chamar a IA)', async () => {
    ...
    const response = await postMessage(purchase.id, otherToken, { problem: validProblem });
  ...
  it('compra com garantia ACTIVE → 200 e status ACTIVE enviado à IA', async () => {
    const { user, token } = await createUserWithToken();
    const purchase = await createPurchase(user.id);
    await createWarranty(purchase.id, daysFromToday(-10), daysFromToday(10));

    const response = await postMessage(purchase.id, token, { problem: validProblem });
  ...
  it('provider mockado retornando mensagem válida → 200 com o schema exato', async () => {
    const { user, token } = await createUserWithToken();
    const purchase = await createPurchase(user.id);
  ...
  it('o problema enviado à IA corresponde ao informado pelo usuário (trim)', async () => {
    const { user, token } = await createUserWithToken();
    const purchase = await createPurchase(user.id);
  ...
  it('message vazia → 503 AI_INVALID_RESPONSE', async () => {
    const { user, token } = await createUserWithToken();
    const purchase = await createPurchase(user.id);

    nextResponse = { kind: 'value', value: { message: '   ' } };
  ...
  it('message abaixo de 20 caracteres → 503 AI_INVALID_RESPONSE', async () => {
    const { user, token } = await createUserWithToken();
    const purchase = await createPurchase(user.id);

    nextResponse = { kind: 'value', value: { message: 'Mensagem curta.' } };
  ...
  it('message acima de 2000 caracteres → 503 AI_INVALID_RESPONSE', async () => {
    const { user, token } = await createUserWithToken();
    const purchase = await createPurchase(user.id);

    nextResponse = { kind: 'value', value: { message: 'a'.repeat(2001) } };
  ...
  it('resposta da IA inválida (schema incompatível) → 503 AI_INVALID_RESPONSE', async () => {
    ...
    nextResponse = { kind: 'value', value: { text: 'só um campo errado' } };
  ...
  it('envia à IA somente os campos esperados do contexto', async () => {
    const { user, token } = await createUserWithToken();
    const purchase = await createPurchase(user.id, {
    ...
    // Internal data never leaves the backend.
